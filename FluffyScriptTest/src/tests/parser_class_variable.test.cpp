...
namespace fluffy { namespace testing {
	...
	TEST_F(ParserClassVariableTest, TestVariableI32)
	{
		...
		EXPECT_EQ(classDecl->variableList[0]->type, ast::ClassMemberType_e::Variable);
		...
		EXPECT_EQ(classDecl->variableList[0]->accessModifier, ast::ClassMemberAccessModifier_e::Private);
		EXPECT_EQ(classDecl->variableList[0]->typeDecl->typeID, ast::TypeDeclID_e::I32);
	}
	...
	{
		parser->loadSource("class Foo { const Foo: i32 = ; }");
		parser->nextToken();
	...
	{
		parser->loadSource("class Foo { const ref Foo: i32 = ; }");
		parser->nextToken();
